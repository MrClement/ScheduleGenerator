...
public class MultiDayCSVPeriodWriter {
	...
	public MultiDayCSVPeriodWriter(String daysOffFile, CurrentDate startDate, CurrentDate endDate,
			CurrentDate earlyLimit, CurrentDate lateLimit) {
	...
	public void generateCSVFile(String filename,
			...
			boolean sixth,
			int[] sixthPrefs) {
		PeriodCSVWriter writer;
		char dayType = 'A';
		int dayAdjust = 0;
		try {
			writer = new PeriodCSVWriter(filename, s);
			writer.writeHeader();
			for (CurrentDate c : daysOn) {
				...
				if (c.isAfterOrEqual(earlyLimit) && (c.isBefore(lateLimit) || c.equals(lateLimit))) {
					writer.writeDayToFile(currentDayType, c, periodsToInclude, periodNames, singleBox, sixthPrefs,
							includeBreaksAndLunch, midSchoolElective, sixth);
				}
